      ...
      tools={{
        addToCart: {
          layoutComponent: ({ message, addToolResult }) => {
            ...
            // Derive inputs only when available (guard against undefined)
            const input = (message.state === 'input-available' ? message.input : {}) as Hit;
        ...
        algolia_search_index: {
          layoutComponent: ({
            ...
            const [topHits, setTopHits] = React.useState<Hit[]>([]);
            ...
            React.useEffect(() => {
              if (message.state === 'input-available' && message.input?.query) {
                ...
                // Only clear if it's a different query
                if (q !== query) {
                  setQuery(q);
                  setTopHits([]);
            ...
            }, [message.state, message.input?.query, query, setIndexUiState]);
            ...
            switch (message.state) {
              ...
              case 'output-available':
                const persistedItems = message.output?.items as Hit[] | undefined;
                const hitsFromOutput = persistedItems || topHits
                  ...
                  return (
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        {hitsFromOutput.map((item: any) => (
                          <ItemComponent key={item.objectID} item={item} />
                        ))}
                        {hitsFromOutput.length === 0 && (
