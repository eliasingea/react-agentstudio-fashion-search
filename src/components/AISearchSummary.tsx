  ...
  try {
    const input = {
      messages: [
        {
          role: "user",
          content: `Generate a search summary for the query: "${query}". Here are the top results: ${JSON.stringify(results.slice(0, 5).map(r => ({title: r.title, description: r.description, brand: r.brand, price: r.price})))}`
        }
      ]
    };
