...
export function ProductPage() {
  ...
  const handleAddToCart = () => {
    ...
    // Get the most recent queryID for this product from localStorage
    let queryID: string | undefined;
    try {
      const clickedItems = localStorage.getItem('clickedItems');
      if (clickedItems) {
        const items = JSON.parse(clickedItems);
        queryID = items[product.objectID];
      }
    } catch (error) {
      console.error('Error retrieving queryID:', error);
    }
    
    // Send conversion event to Algolia if we have a queryID
    if (queryID) {
